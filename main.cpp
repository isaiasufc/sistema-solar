...
Planet nep(2.3, 53.6, 0, 0.54, 28.32, 0);	//Neturno
Planet lun(.40, 3, 0, 5.40, 0, 0);			
Planet pho(.20, 1.8, 0, 2.30, 0, 0);		
Planet dei(.24, 2.4, 0, 3.60, 0, 0);		
Planet eur(.24, 4, 0, 4.40, 0, 0);			
Planet gan(.24, 4.7, 0, 5.00, 0, 0);
Planet cal(.24, 5.3, 0, 2.30, 0, 0);		
Planet tit(.75, 3.7, 0, 2.40, 0, 0);		
Planet nix(.10, 1.5, 0, 5.00, 0, 0);		
Planet puc(.26, 2.9, 0, 7.00, 0, 0);		  
Planet tri(.36, 3.2, 0, 3.40, 0, 0);		  
...
void animate(int n){
	if (isAnimate){
		...
		if (mer, ven, ear, mar, jup, sat, ura, nep, lun, pho, dei, eur, gan, cal, tit, nix, puc, tri.orbit > 360.0){
			mer, ven, ear, mar, jup, sat, ura, nep, lun, pho, dei, eur, gan, cal, tit, nix, puc, tri.orbit -= 360.0;
		}
		mer.axisAni += 10.0;
		ven.axisAni += 10.0;
		ear.axisAni += 10.0;
